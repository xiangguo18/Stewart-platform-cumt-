...
while True:
    try:
        ...
        hex_string = input("请输入不带 CRC 校验码的十六进制形式的 Modbus RTU 报文，以空格分隔每个字节，输入 q 退出：\n")
        ...
        hex_crc_bytes = bytearray.fromhex(hex_string.replace(" ", ""))
        #send_modbus_rtu_request(hex_crc_bytes)
    except ValueError:
        print("请输入有效的十六进制字符串。")
